...
fn simple_integrity_test() {
    ...
    let primary_block = primary::PrimaryBlockBuilder::default()
        ...
        .build()
        .unwrap();
    let cbor_primary = minicbor_serde::to_vec(&primary_block).unwrap();
    ...
    let cbor_payload = minicbor_serde::to_vec(&payload_block).unwrap();
    ...
    let cbor_bib = minicbor_serde::to_vec(&block_integrity_block).unwrap();
