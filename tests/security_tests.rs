...
fn simple_integrity_test() {
    ...
    let primary_block = primary::PrimaryBlockBuilder::default()
        ...
        .build()
        .unwrap();
    let cbor_primary = cbor4ii::serde::to_vec(Vec::new(), &primary_block).unwrap();
    ...
    let cbor_payload = cbor4ii::serde::to_vec(Vec::new(), &payload_block).unwrap();
    ...
    let cbor_bib = cbor4ii::serde::to_vec(Vec::new(), &block_integrity_block).unwrap();
