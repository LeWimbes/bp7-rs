...
fn simple_integrity_test() {
    ...
    let primary_block = primary::PrimaryBlockBuilder::default()
        ...
        .build()
        .unwrap();
    let cbor_primary = bp7::helpers::to_cbor_vec(&primary_block).unwrap();
    ...
    let cbor_payload = bp7::helpers::to_cbor_vec(&payload_block).unwrap();
    ...
    let cbor_bib = bp7::helpers::to_cbor_vec(&block_integrity_block).unwrap();
