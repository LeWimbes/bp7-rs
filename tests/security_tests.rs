...
fn simple_integrity_test() {
    ...
    let primary_block = primary::PrimaryBlockBuilder::default()
        ...
        .build()
        .unwrap();
    let cbor_primary = serde_cbor_2::to_vec(&primary_block).unwrap();
    ...
    let cbor_payload = serde_cbor_2::to_vec(&payload_block).unwrap();
    ...
    let cbor_bib = serde_cbor_2::to_vec(&block_integrity_block).unwrap();
