...
fn status_report_tests() {
    ...
    let encoded_sr1 = cbor4ii::serde::to_vec(Vec::new(), &sr1).unwrap();

    let sr1_dec: StatusReport = cbor4ii::serde::from_slice(&encoded_sr1).unwrap();
    ...
    let encoded_sr2 = cbor4ii::serde::to_vec(Vec::new(), &sr2).unwrap();

    let sr2_dec: StatusReport = cbor4ii::serde::from_slice(&encoded_sr2).unwrap();
