...
fn status_report_tests() {
    ...
    let encoded_sr1 = serde_cbor_2::to_vec(&sr1).unwrap();

    let sr1_dec: StatusReport = serde_cbor_2::from_slice(&encoded_sr1).unwrap();
    ...
    let encoded_sr2 = serde_cbor_2::to_vec(&sr2).unwrap();

    let sr2_dec: StatusReport = serde_cbor_2::from_slice(&encoded_sr2).unwrap();
