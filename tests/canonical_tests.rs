...
fn canonical_data_tests() {
    ...
    let encoded_data = bp7::helpers::to_cbor_vec(&data).expect("encoding error");
    let decoded_data: CanonicalData =
        bp7::helpers::from_cbor_slice(&encoded_data).expect("decoding error");
    assert_eq!(data, decoded_data);

    let bundleage = dbg!(CanonicalData::BundleAge(23));
    let encoded_bundleage = bp7::helpers::to_cbor_vec(&bundleage).expect("encoding error");
    let decoded_bundleage: CanonicalData =
        bp7::helpers::from_cbor_slice(&encoded_bundleage).expect("decoding error");
    assert_eq!(bundleage, decoded_bundleage);

    let hopcount = CanonicalData::HopCount(23, 42);
    let encoded_hopcount = bp7::helpers::to_cbor_vec(&hopcount).expect("encoding error");
    let decoded_hopcount: CanonicalData =
        bp7::helpers::from_cbor_slice(&encoded_hopcount).expect("decoding error");
    assert_eq!(hopcount, decoded_hopcount);

    let previous = CanonicalData::PreviousNode("dtn://node1".try_into().unwrap());
    let encoded_previous = bp7::helpers::to_cbor_vec(&previous).expect("encoding error");
    let decoded_previous: CanonicalData =
        bp7::helpers::from_cbor_slice(&encoded_previous).expect("decoding error");
