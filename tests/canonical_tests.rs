...
fn canonical_data_tests() {
    ...
    let encoded_data = cbor4ii::serde::to_vec(Vec::new(), &data).expect("encoding error");
    let decoded_data: CanonicalData =
        cbor4ii::serde::from_slice(&encoded_data).expect("decoding error");
    assert_eq!(data, decoded_data);

    let bundleage = dbg!(CanonicalData::BundleAge(23));
    let encoded_bundleage = cbor4ii::serde::to_vec(Vec::new(), &bundleage).expect("encoding error");
    let decoded_bundleage: CanonicalData =
        cbor4ii::serde::from_slice(&encoded_bundleage).expect("decoding error");
    assert_eq!(bundleage, decoded_bundleage);

    let hopcount = CanonicalData::HopCount(23, 42);
    let encoded_hopcount = cbor4ii::serde::to_vec(Vec::new(), &hopcount).expect("encoding error");
    let decoded_hopcount: CanonicalData =
        cbor4ii::serde::from_slice(&encoded_hopcount).expect("decoding error");
    assert_eq!(hopcount, decoded_hopcount);

    let previous = CanonicalData::PreviousNode("dtn://node1".try_into().unwrap());
    let encoded_previous = cbor4ii::serde::to_vec(Vec::new(), &previous).expect("encoding error");
    let decoded_previous: CanonicalData =
        cbor4ii::serde::from_slice(&encoded_previous).expect("decoding error");
