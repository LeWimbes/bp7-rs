...
impl IntegrityProtectedPlaintext {
    ...
    pub fn create(&mut self, payload_block: &CanonicalBlock) -> ByteBuffer {
        ...
        self.security_target_contents = helpers::to_cbor_vec(&payload_block.data()).unwrap();
        ...
        if !matches!(payload_block.data(), CanonicalData::Data(_)) {
            ...
            self.security_target_contents = helpers::to_cbor_vec(&temp_bytes).unwrap();
        }

        let mut ippt = Vec::<u8>::new();
        ippt.append(
            &mut helpers::to_cbor_vec(&self.scope_flags)
    ...
    ) -> Result<ByteBuffer, ciborium::ser::Error<std::io::Error>> {
        let mut header = Vec::<u8>::new();
        header.append(&mut helpers::to_cbor_vec(&payload_block.block_type)?);
        header.append(&mut helpers::to_cbor_vec(&payload_block.block_number)?);
        header.append(&mut helpers::to_cbor_vec(
            &payload_block.block_control_flags,
        )?);
        //header.append(&mut helpers::to_cbor_vec(&payload_block.crc.to_code())?); //TODO: check if not needed?
        Ok(header)
    }
    ...
    fn construct_security_header(
        ...
        let mut header = Vec::<u8>::new();
        header.append(&mut helpers::to_cbor_vec(&security_block_parameter.0)?);
        header.append(&mut helpers::to_cbor_vec(&security_block_parameter.1)?);
        header.append(&mut helpers::to_cbor_vec(&security_block_parameter.2)?);
        Ok(header)
    }
}
...
impl IntegrityBlock {
    ...
    pub fn to_cbor(&self) -> ByteBuffer {
        ...
        cbor_format.append(&mut helpers::to_cbor_vec(&self.security_targets).unwrap());
        cbor_format.append(&mut helpers::to_cbor_vec(&self.security_context_id).unwrap());
        cbor_format.append(&mut helpers::to_cbor_vec(&self.security_context_flags).unwrap());
        cbor_format.append(&mut helpers::to_cbor_vec(&self.security_source).unwrap());
        cbor_format.append(&mut helpers::to_cbor_vec(&self.security_context_parameters).unwrap());
        ...
        cbor_format.append(&mut helpers::to_cbor_vec(&res).unwrap());
        cbor_format
    }
}
