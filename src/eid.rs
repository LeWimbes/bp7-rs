        ...
        impl<'de> Visitor<'de> for EndpointIDVisitor {
                ...
                V: SeqAccess<'de>,
            {
                ...
                if eid_type == ENDPOINT_URI_SCHEME_DTN {
                    ...
                    let peek_next = seq.next_element::<cbor4ii::core::Value>()?;
                    if peek_next.is_none() {
                        return Err(de::Error::invalid_length(1, &self));
                    }
                    let peek_next = peek_next.unwrap();
                    if let cbor4ii::core::Value::Text(name) = peek_next {
                        // This is a dtn address
                        Ok(EndpointID::Dtn(eid_type, DtnAddress(name)))
                    } else if let cbor4ii::core::Value::Integer(code) = peek_next {
...
mod tests {
    ...
    fn serialize_deserialize_tests(eid: EndpointID) {
        let encoded_eid =
            cbor4ii::serde::to_vec(Vec::new(), &eid).expect("Error serializing packet as cbor.");
        println!("{:02x?}", &encoded_eid);
        assert_eq!(
            eid,
            cbor4ii::serde::from_slice(&encoded_eid).expect("Decoding packet failed")
        );
    }
    ...
    fn test_ser_eid(cbor_eid: &[u8]) -> String {
        let deserialized: EndpointID = cbor4ii::serde::from_slice(cbor_eid).unwrap();
