        ...
        match self.data {
            ...
            _ => {
                seq.serialize_element(&serde_bytes::Bytes::new(
                    &minicbor_serde::to_vec(&self.data).unwrap(),
                ))?;
            }
        };
                ...
                V: SeqAccess<'de>,
            {
                ...
                let data = if block_type == PAYLOAD_BLOCK {
                    ...
                    CanonicalData::BundleAge(
                        minicbor_serde::from_slice::<u64>(&raw_payload).map_err(|err| {
                            de::Error::custom(format!("error decoding bundle age block: {}", err))
                        })?,
                    )
                } else if block_type == HOP_COUNT_BLOCK {
                    let hc: (u8, u8) = minicbor_serde::from_slice(&raw_payload).map_err(|err| {
                    ...
                    CanonicalData::PreviousNode(minicbor_serde::from_slice(&raw_payload).map_err(
                        |err| {
...
impl CanonicalData {
    pub fn to_cbor(&self) -> ByteBuffer {
        minicbor_serde::to_vec(self).expect("CanonicalData encoding error")
    }
}
