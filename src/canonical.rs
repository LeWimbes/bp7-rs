        ...
        match self.data {
            ...
            _ => {
                seq.serialize_element(&serde_bytes::Bytes::new(
                    &cbor4ii::serde::to_vec(Vec::new(), &self.data).unwrap(),
                ))?;
            }
        };
                ...
                V: SeqAccess<'de>,
            {
                ...
                let data = if block_type == PAYLOAD_BLOCK {
                    ...
                    CanonicalData::BundleAge(
                        cbor4ii::serde::from_slice::<u64>(&raw_payload).map_err(|err| {
                            de::Error::custom(format!("error decoding bundle age block: {}", err))
                        })?,
                    )
                } else if block_type == HOP_COUNT_BLOCK {
                    let hc: (u8, u8) = cbor4ii::serde::from_slice(&raw_payload).map_err(|err| {
                    ...
                    CanonicalData::PreviousNode(cbor4ii::serde::from_slice(&raw_payload).map_err(
                        |err| {
...
impl Block for CanonicalBlock {
    fn to_cbor(&self) -> ByteBuffer {
        cbor4ii::serde::to_vec(Vec::new(), &self).unwrap()
    }
}
...
impl CanonicalData {
    pub fn to_cbor(&self) -> ByteBuffer {
        cbor4ii::serde::to_vec(Vec::new(), &self).expect("CanonicalData encoding error")
    }
}
